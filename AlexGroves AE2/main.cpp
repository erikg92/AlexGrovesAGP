...
char g_Name[100] = "Alexander Groves AE2\0";
...
//const buffer structs. pack to 16 bytes. Don't let any single element element cross a 16 byte boundary
...
HRESULT InitialiseWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_hWnd = CreateWindow(Name, g_Name, WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		{
			//degrees += 5.0f;
			model[0]->IncXRotation(2);
			model[0]->IncYRotation(2);
			model[0]->IncZRotation(2);

			model[1]->IncXRotation(2);
			model[1]->IncYRotation(2);
			model[1]->IncZRotation(2);
		}
...
HRESULT InitialiseGraphics()//03-01
{
	...
	model[0] = new Model(g_pD3DDevice, g_pImmediateContext);
	model[0]->LoadObjModel("assets/cube.obj");
	model[0]->SetXPos(3);

	model[1] = new Model(g_pD3DDevice, g_pImmediateContext);
	model[1]->LoadObjModel("assets/Sphere.obj");
	model[1]->SetXPos(-3);
	...
	POS_COL_TEX_NORM_VERTEX vertices[] =
	{
		...
		{ XMFLOAT3(-1.0f,-1.0f, 1.0f), XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f), XMFLOAT2(0.0, 1.0f), XMFLOAT3(0.0f, 0.0f, 1.0f)},
	...
	D3DX11CreateShaderResourceViewFromFile(g_pD3DDevice, "assets/goku.bmp", NULL, NULL, &g_pTexture0, NULL);
	...
	camera = new Camera(0.0f, 0.0f, -10.0f, 0.0f);
...
*/
	...
	model[0]->Draw(&view, &projection);
	model[1]->Draw(&view, &projection);
	...
	//world *= XMMatrixTranslation(0, 1, 14);
	//projection = XMMatrixPerspectiveFovLH(XMConvertToRadians(45.0), 640.0 / 480.0, 1.0, 100.0);
	//view = camera->GetViewMatrix();
	//cb0_values.WorldViewProjection = world * view * projection;
	...
	//RENDER HERE
	degrees += 0.001;
	fps = std::to_string(degrees);
	
	g_2DText->AddText("FPS: " +fps, -1.0, +1.0, .2);
